    ...
    def __init__(
        ...
        if incoming_id:
            super().__init__(
                ...
                detail=f"Unable to find the {model.__name__} with id {incoming_id}.",
                headers=headers,
            )
            return
...
class DateNotFoundException(HTTPException, Generic[ModelType]):
    def __init__(
            self,
            model: Type[ModelType],
        ...
        if incoming_date:
            super().__init__(
                status_code=status.HTTP_404_NOT_FOUND,
                detail=f"Unable to find the {model.__name__} with date {incoming_date}.",
                headers=headers,
            )
            return
        ...
        super().__init__(
            status_code=status.HTTP_404_NOT_FOUND,
            detail=f"{model.__name__} date not found.",
            headers=headers,
        )
